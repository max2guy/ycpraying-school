...
## 현재 상태
- 브랜치: `main`
- 최신 커밋: `231805a` — `feat: 필사 구절 배분을 말씀의 흐름(문단 단위)에 맞춰 재조정`
...
일일미션 `MISSION_SCHEDULE`을 사용자 피드백에 따라 4차례 조정했다 (커밋 순서대로):
...
4. **`231805a`** — 사용자가 "막 나누지 말고 말씀의 흐름에 따라서 나누기"라고 요청 → 기계적 절수 등분(8/8/8/8/8/7) 대신 사도행전 2장의 **내러티브 구조(문단 단위)**로 재배분:
   - 1일차 1-13절(13절) — 성령강림과 방언, 사람들의 반응
   - 2일차 14-21절(8절) — 베드로의 설교 ① 요엘의 예언 인용 (암송 구절 17절 포함)
   - 3일차 22-28절(7절) — 베드로의 설교 ② 그리스도의 죽음과 부활 증거, 다윗의 시 인용
   - 4일차 29-36절(8절) — 베드로의 설교 ③ 다윗 예언 해석, "주와 그리스도" 결론
   - 5일차 37-41절(5절) — 청중의 회개·세례, 삼천 명 회심
   - 6일차 42-47절(6절) — 초대교회의 공동체 생활
   - 13+8+7+8+5+6=47절로 전체 분량과 일치, 각 날짜가 하나의 완결된 이야기 단위가 되도록 함
   - `desc`에 각 문단의 주제를 짧게 덧붙임 (예: "성령강림과 방언 — 13절을 손으로 직접 필사한 후...")
